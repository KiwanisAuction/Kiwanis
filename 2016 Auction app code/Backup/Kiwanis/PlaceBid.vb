...
Public Class PlaceBid
    ...
    Private Sub loadActiveBlock(ByVal strBlockType As String)
        ...
        SQLConnLAB.ConnectionString = Conn_CMD
    ...
    Private Sub displaySuccessBid(ByVal ItemID As Integer, ByVal highBid As Integer)
        ...
        Try
            ...
            Else
                lblMessageBox.Text = "UNSUCCESSFUL BID!: Another bid has been placed before the current attempt." ' Changed from "Unsuccessful Bid: Another bidder has placed a bid before you"
            End If
        Catch ex As Exception
            MessageBox.Show(ex.ToString)
        Finally
    ...
    Private Sub BidderAreaPhone_txt_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BidderAreaPhone_txt.TextChanged
        If BidderAreaPhone_txt.TextLength = 3 Then
            BidderPrefixPhone_txt.Focus()
        End If
        ' Verify area code number is NUMERIC
        If IsValidAlphaNum(BidderAreaPhone_txt.Text) = True Then
            BidderAreaPhone_txt.BackColor = Color.LightGreen
            lblMessageBox.Text = "Letters are not valid input for phone number."
            PhoneIsNum_sw = 1
        Else
            BidderAreaPhone_txt.BackColor = Color.White
            lblMessageBox.Text = ""
            PhoneIsNum_sw = 0
        End If
    ...
    Private Sub BidderPrefixPhone_txt_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BidderPrefixPhone_txt.TextChanged
        If BidderPrefixPhone_txt.TextLength >= 3 Then
            BidderLinePhone_txt.Focus()
        ElseIf BidderPrefixPhone_txt.TextLength < 1 Then
            BidderAreaPhone_txt.Focus()
        End If
        ' Verify prefix number is NUMERIC
        If IsValidAlphaNum(BidderPrefixPhone_txt.Text) = True Then
            BidderPrefixPhone_txt.BackColor = Color.LightGreen
            lblMessageBox.Text = "Letters are not valid input for phone number."
            PhoneIsNum_sw = 1
        Else
